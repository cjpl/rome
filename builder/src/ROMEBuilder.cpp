/********************************************************************
  ...
  $Log$
  Revision 1.193  2005/07/01 15:42:07  schneebeli_m
  implemented SPField arrays

  Revision 1.192  2005/07/01 08:20:22  schneebeli_m
  added byte swapping of TID_STRUCTS

  Revision 1.191  2005/07/01 07:38:36  schneebeli_m
  commit problems

  Revision 1.189  2005/07/01 07:29:41  schneebeli_m
  changes of Folders
  ...
  Revision 1.170  2005/06/01 11:09:02  schneebeli_m
  error
  ...
  Revision 1.149  2005/05/06 22:38:47  sawada
  small bug fix.
...
bool ROMEBuilder::WriteFolderH() {
   ...
   for (int iFold=0;iFold<numOfFolder;iFold++) {
      ...
      buffer.AppendFormatted("#include <ROMEFolder.h>\n");
      ...
      // Class
      if (folderUserCode[iFold])
         buffer.AppendFormatted("\nclass %s%s_Base : public ROMEFolder\n",shortCut.Data(),folderName[iFold].Data());
      else
         buffer.AppendFormatted("\nclass %s%s : public ROMEFolder\n",shortCut.Data(),folderName[iFold].Data());
      buffer.AppendFormatted("{\n");
      ...
      for (i=0;i<numOfValue[iFold];i++) {
         if (isFolder(valueType[iFold][i].Data())){
            ...
            if (valueArray[iFold][i]=="1"){
               ...
               buffer.AppendFormatted("      %s->SetParentFolder(this);\n",valueName[iFold][i].Data());
            }
            else {
               buffer.AppendFormatted("      %s = new TClonesArray(\"%s\");\n",valueName[iFold][i].Data(),tmp.Data());
               if (valueArray[iFold][i]!="variable")
                  buffer.AppendFormatted("      Allocate%s(%s);\n",valueName[iFold][i].Data(),valueArray[iFold][i].Data());
            }
         }
         else {
            if (valueArray[iFold][i]=="1")
               ...
               buffer.AppendFormatted("      for (int i%d=0;i%d<%s;i%d++) %s[i%d] = %s;\n",i,i,valueArray[iFold][i].Data(),i,valueName[iFold][i].Data(),i,valueInit[iFold][i].Data());
            }
         }
      }
      buffer.AppendFormatted("      fModified = false;\n");
      buffer.AppendFormatted("      fParentFolder = 0;\n");
      buffer.AppendFormatted("   };\n");
      buffer.AppendFormatted("\n");
      ...
      for (i=0;i<numOfValue[iFold];i++) {
         ...
         if (valueArray[iFold][i]=="1") {
            ...
            if (valueType[iFold][i]=="TRef") {
               format.SetFormatted("   void Set%%sAt%%%ds(int index,%%-%ds %%s_value%%%ds) { %%s[index]%%%ds = %%s_value;%%%ds SetModified(true); };\n",lb,typeLen,lb,lb,lb);
               buffer.AppendFormatted(format.Data(),valueName[iFold][i].Data(),"","TObject*",valueName[iFold][i].Data(),"",valueName[iFold][i].Data(),"",valueName[iFold][i].Data(),"");
            }
            else {
               format.SetFormatted("   void Set%%sAt%%%ds(int index,%%-%ds %%s_value%%%ds) { %%s[index]%%%ds = %%s_value;%%%ds SetModified(true); };\n",lb,typeLen,lb,lb,lb);
      ...
      for (i=0;i<numOfValue[iFold];i++) {
         ...
         if (valueArray[iFold][i]=="1") {
            ...
            if (valueType[iFold][i]=="TRef") {
               format.SetFormatted("   void Add%%sAt%%%ds(int index,%%-%ds %%s_value%%%ds) { %%s[index]%%%ds += %%s_value;%%%ds SetModified(true); };\n",lb,typeLen,lb,lb,lb);
               buffer.AppendFormatted(format.Data(),valueName[iFold][i].Data(),"","TObject*",valueName[iFold][i].Data(),"",valueName[iFold][i].Data(),"",valueName[iFold][i].Data(),"");
            }
            else {
               format.SetFormatted("   void Add%%sAt%%%ds(int index,%%-%ds %%s_value%%%ds) { %%s[index]%%%ds += %%s_value;%%%ds SetModified(true); };\n",lb,typeLen,lb,lb,lb);
      ...
      // Allocate
      for (i=0;i<numOfValue[iFold];i++) {
         if (isFolder(valueType[iFold][i].Data()) && valueArray[iFold][i]=="variable"){
            tmp = valueType[iFold][i];
            tmp.ReplaceAll("*","");
            ...
            buffer.AppendFormatted("         ((%s)%s->At(i))->SetParentFolder(this);\n",valueType[iFold][i].Data(),valueName[iFold][i].Data());
            buffer.AppendFormatted("      }\n");
            buffer.AppendFormatted("   }\n");
         }
      }
      ...
      // Allocate
      for (i=0;i<numOfValue[iFold];i++) {
         if (isFolder(valueType[iFold][i].Data()) && valueArray[iFold][i]!="1" && valueArray[iFold][i]!="variable"){
            tmp = valueType[iFold][i];
            tmp.ReplaceAll("*","");
            buffer.AppendFormatted("   void Allocate%s(int number) {\n",valueName[iFold][i].Data());
            buffer.AppendFormatted("      int i;\n");
...
bool ROMEBuilder::WriteSteeringClass(ROMEString &buffer,int numSteer,int numTask,int tab) {
   ...
   for (j=0;j<numOfSteerFields[numTask][numSteer];j++) {
      if (steerFieldArraySize[numTask][numSteer][j]=="1") {
         ...
         format.SetFormatted("%%s   %%-%ds f%%s[%%s];%%%ds %%s\n",typeLen,nameLen+3-steerFieldName[numTask][numSteer][j].Length());
   ...
   for (i=0;i<numOfSteerChildren[numTask][numSteer];i++) {
      format.SetFormatted("%%s   %%-%ds *f%%s;%%%ds // Handle to %%s Class\n",typeLen-1,nameLen-steerName[numTask][steerChildren[numTask][numSteer][i]].Length()-steerFieldArraySize[numTask][numSteer][j].Length());
   ...
   for (j=0;j<numOfSteerFields[numTask][numSteer];j++) {
      if (steerFieldArraySize[numTask][numSteer][j]=="1") {
         format.SetFormatted("%%s   %%-%ds Get%%s()%%%ds { return f%%s; };\n",typeLen,nameLen+9-steerFieldName[numTask][numSteer][j].Length());
...
bool ROMEBuilder::WriteSteeringConfigRead(ROMEString &buffer,int numSteer,int numTask,ROMEXML *xml,ROMEString& path,ROMEString& pointer,ROMEString& classPath) {
   ...
   for (k=0;k<numOfSteerFields[numTask][numSteer];k++) {
      ...
      if (steerFieldArraySize[numTask][numSteer][k]=="1") {
         ...
         buffer.AppendFormatted("      buf.SetFormatted(%s/SteeringParameterField[SPName='%s']/SPArray[SPArrayIndex=%%d]/SPArrayValue\",i);\n",path.Data(),steerFieldName[numTask][numSteer][k].Data(),pointer.Data(),steerFieldName[numTask][numSteer][k].Data());
         buffer.AppendFormatted("      xml->GetPathValue(buf.Data(),%s->f%s[i],\"\");\n",pointer.Data(),steerFieldName[numTask][numSteer][k].Data());
         buffer.AppendFormatted("      if (%s->f%s[i]==\"\")\n",pointer.Data(),steerFieldName[numTask][numSteer][k].Data());
         buffer.AppendFormatted("         %s->f%sModified[i] = false;\n",pointer.Data(),steerFieldName[numTask][numSteer][k].Data());
         buffer.AppendFormatted("      else {\n");
         buffer.AppendFormatted("         %s->f%sModified[i] = true;\n",pointer.Data(),steerFieldName[numTask][numSteer][k].Data());
         buffer.AppendFormatted("         %s->f%sArrayModified = true;\n",pointer.Data(),steerFieldName[numTask][numSteer][k].Data());
         buffer.AppendFormatted("         %sModified = true;\n",pointer.Data());
         buffer.AppendFormatted("      }\n");
         buffer.AppendFormatted("   }\n");
      }
...
bool ROMEBuilder::WriteSteeringConfigSet(ROMEString &buffer,int numSteer,int numTask,ROMEString& pointer,ROMEString& steerPointer) {
   ...
   for (k=0;k<numOfSteerFields[numTask][numSteer];k++) {
      ...
      if (steerFieldArraySize[numTask][numSteer][k]=="1") {
         ...
         buffer.AppendFormatted("   for (i=0;i<%s;i++) {\n",steerFieldArraySize[numTask][numSteer][k].Data());
         buffer.AppendFormatted("      if (fConfigData[modIndex]%s->f%sModified[i]) {\n",pointer.Data(),steerFieldName[numTask][numSteer][k].Data());
         value.SetFormatted("fConfigData[index]%s->f%s[i]",pointer.Data(),steerFieldName[numTask][numSteer][k].Data());
         setValue(&decodedValue,"",value.Data(),steerFieldType[numTask][numSteer][k].Data(),1);
         if (steerFieldType[numTask][numSteer][k]=="std::string")
            buffer.AppendFormatted("         %s->Set%sAt(i,%s.Data());\n",steerPointer.Data(),steerFieldName[numTask][numSteer][k].Data(),value.Data());
         else
            buffer.AppendFormatted("         %s->Set%sAt(i,(%s)%s);\n",steerPointer.Data(),steerFieldName[numTask][numSteer][k].Data(),steerFieldType[numTask][numSteer][k].Data(),decodedValue.Data());
         buffer.AppendFormatted("      }\n");
         buffer.AppendFormatted("   }\n");
      }
   }
...
bool ROMEBuilder::WriteSteeringConfigWrite(ROMEString &buffer,int numSteer,int numTask,ROMEString& pointer,ROMEString& steerPointer,int tab) {
   ...
   for (k=0;k<numOfSteerFields[numTask][numSteer];k++) {
      ...
      if (steerFieldArraySize[numTask][numSteer][k]=="1") {
         ...
         buffer.AppendFormatted("%s            xml->StartElement(\"SPArray\");\n",blank.Data());
         buffer.AppendFormatted("%s            value.SetFormatted(\"%%d\",i);\n",blank.Data());
         buffer.AppendFormatted("%s            xml->WriteElement(\"SPArrayIndex\",value.Data());\n",blank.Data());
         buffer.AppendFormatted("%s            if (index==0) {\n",blank.Data());
         GetFormat(&value,steerFieldType[numTask][numSteer][k].Data());
         if (steerFieldType[numTask][numSteer][k]=="TString")
         ...
         buffer.AppendFormatted("%s               xml->WriteElement(\"SPArrayValue\",value.Data());\n",blank.Data());
         buffer.AppendFormatted("%s            }\n",blank.Data());
         buffer.AppendFormatted("%s            else if (%s->f%sModified)\n",blank.Data(),pointer.Data(),steerFieldName[numTask][numSteer][k].Data());
         buffer.AppendFormatted("%s               xml->WriteElement(\"SPArrayValue\",%s->f%s[i].Data());\n",blank.Data(),pointer.Data(),steerFieldName[numTask][numSteer][k].Data());
         buffer.AppendFormatted("%s            xml->EndElement();\n",blank.Data());
         buffer.AppendFormatted("%s         }\n",blank.Data());
         buffer.AppendFormatted("%s      }\n",blank.Data());
         buffer.AppendFormatted("%s      xml->EndElement();\n",blank.Data());
...
void ROMEBuilder::WriteReadDataBaseFolder(ROMEString &buffer,int numFolder,int type) {
   ...
   for (j=0;j<numOfValue[numFolder];j++) {
      if (folderArray[numFolder]=="1" && type==1 || folderArray[numFolder]!="1" && type==2 && !folderSupport[j]) {
         ...
         buffer.AppendFormatted(");\n"); 
         buffer.AppendFormatted("   if (name.Length() && path.Length()){\n");
         ...
         if (valueArray[numFolder][j]=="1") {
            ...
            buffer.AppendFormatted("%s      for (j=0;j<%s;j++) {\n",blank.Data(),valueArray[numFolder][j].Data());
   ...
   for (i=0;i<numOfFolder;i++) {
      if (numOfValue[i] > 0 && !folderSupport[i]) {
         if (folderArray[i]!="1" && folderArray[i]!="variable")
            buffer.AppendFormatted("   gAnalyzer->Allocate%s(%s);\n",folderName[i].Data(),folderArray[i].Data());
      }
   }
   buffer.AppendFormatted("};\n\n");
...
// ------------------
   ...
   for (i=0;i<numOfFolder;i++) {
      if (folderUserCode[i]) {
         buffer.AppendFormatted("obj/%s%s.obj: src/framework/%s%s.cpp include/framework/%s%s.h $(ROMESYS)/include/ROMEFolder.h",shortCut.Data(),folderName[i].Data(),shortCut.Data(),folderName[i].Data(),shortCut.Data(),folderName[i].Data());
	      buffer.AppendFormatted(" include/framework/%s%s_Base.h",shortCut.Data(),folderName[i].Data());
         buffer.AppendFormatted("\n");
   ...
   buffer.AppendFormatted("obj/ROMEAnalyzer.obj: $(ROMESYS)/src/ROMEAnalyzer.cpp  $(ROMESYS)/include/ROMEAnalyzer.h  $(ROMESYS)/include/ROMETask.h  $(ROMESYS)/include/ROMEFolder.h\n");
   ...
   buffer.AppendFormatted(" $(ROMESYS)/include/ROMETask.h $(ROMESYS)/include/ROMEFolder.h $(ROMESYS)/include/ROMETreeInfo.h $(ROMESYS)/include/ROMEAnalyzer.h include/framework/%sAnalyzer.h $(DictionaryHeaders)\n",shortCut.Data());
...
void ROMEBuilder::WriteDictionaryBat(ROMEString& buffer) 
{
   ...
   buffer.AppendFormatted("ROMETask.h ROMEFolder.h ROMETreeInfo.h ROMEAnalyzer.h include/framework/%sAnalyzer.h\n",shortCut.Data());
   buffer.Append("\0");
