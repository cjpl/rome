                     ...
                     if (histoArraySize[tabSingleObjectTaskIndex[iTab][j]][tabSingleObjectObjectIndex[iTab][j]] == "1") {
                        ...
                        for (k = tabSingleObjectArrayIndexStart[iTab][j]; k <= tabSingleObjectArrayIndexEnd[iTab][j];
                           ...
                           buffer.AppendFormatted("      f%sSingleObject%d_%d = new %s();\n",
                                                  tabSingleObjectName[iTab][j].Data(),j,k,
                                                  histoType[tabSingleObjectTaskIndex[iTab][i]][tabSingleObjectObjectIndex[iTab][i]].Data());
                           buffer.AppendFormatted("      *(f%sSingleObject%d_%d) = *static_cast<%s*>(((TObjArray*)gAnalyzer->GetTaskObjectAt(%d)->GetHistoAt(%d))->At(%d));\n",
                                                  tabSingleObjectName[iTab][j].Data(),j,k,
                                                  histoType[tabSingleObjectTaskIndex[iTab][j]][tabSingleObjectObjectIndex[iTab][j]].Data(),
                                                  tabSingleObjectTaskHierarchyIndex[iTab][j],
                     ...
                     if (graphArraySize[tabSingleObjectTaskIndex[iTab][j]][tabSingleObjectObjectIndex[iTab][j]] == "1") {
                        ...
                        for (k = tabSingleObjectArrayIndexStart[iTab][j]; k <= tabSingleObjectArrayIndexEnd[iTab][j];
                           ...
                           buffer.AppendFormatted("      f%sSingleObject%d_%d = new %s();\n",
                                                  tabSingleObjectName[iTab][j].Data(),j,k,
                                                  graphType[tabSingleObjectTaskIndex[iTab][j]][tabSingleObjectObjectIndex[iTab][j]].Data());
                           buffer.AppendFormatted("      if ((static_cast<%s*>(((TObjArray*)gAnalyzer->GetTaskObjectAt(%d)->GetGraphAt(%d))->At(%d)))->GetN()<=0)\n",
                                                  graphType[tabSingleObjectTaskIndex[iTab][j]][tabSingleObjectObjectIndex[iTab][j]].Data(),
                                                  tabSingleObjectTaskHierarchyIndex[iTab][j],
                                                  tabSingleObjectObjectIndex[iTab][j],k);
                           buffer.AppendFormatted("         f%sSingleObject%d_%d->Set(1);\n",
                                                  tabSingleObjectName[iTab][j].Data(),j,k);
                           buffer.AppendFormatted("      else\n");
                           buffer.AppendFormatted("         *(f%sSingleObject%d_%d) = *static_cast<%s*>(((TObjArray*)gAnalyzer->GetTaskObjectAt(%d)->GetGraphAt(%d))->At(%d));\n",
                                                  tabSingleObjectName[iTab][j].Data(),j,k,
                                                  graphType[tabSingleObjectTaskIndex[iTab][j]][tabSingleObjectObjectIndex[iTab][j]].Data(),
                                                  tabSingleObjectTaskHierarchyIndex[iTab][j],
                                                  tabSingleObjectObjectIndex[iTab][j],k);
               ...
               for (j = 0; j < numOfTabObjectDisplayObjects[iTab][i]; j++) {
                  ...
                  if (tabObjectDisplayType[iTab][i][j].BeginsWith("ROMETG") || tabObjectDisplayType[iTab][i][j].BeginsWith("ROMETC")) {
                     ...
                     if (graphArraySize[tabObjectDisplayTaskIndex[iTab][i]][tabObjectDisplayObjectIndex[iTab][i][j]] == "1") {
                        ...
                        buffer.AppendFormatted("         *static_cast<%s*>(ptr->At(i)) = *static_cast<%s*>(((TObjArray*)gAnalyzer->GetTaskObjectAt(%d)->GetGraphAt(%d))->At(i));\n",
                                               tabObjectDisplayType[iTab][i][j].Data(),
                                               tabObjectDisplayType[iTab][i][j].Data(),
                                               tabObjectDisplayTaskHierarchyIndex[iTab][i],
                                               tabObjectDisplayObjectIndex[iTab][i][j]);
                     ...
                     if (histoArraySize[tabObjectDisplayTaskIndex[iTab][i]][tabObjectDisplayObjectIndex[iTab][i][j]] == "1") {
                        ...
                        buffer.AppendFormatted("         *static_cast<%s*>(ptr->At(i)) = *static_cast<%s*>(((TObjArray*)gAnalyzer->GetTaskObjectAt(%d)->GetHistoAt(%d))->At(i));\n",
                                               tabObjectDisplayType[iTab][i][j].Data(),
                                               tabObjectDisplayType[iTab][i][j].Data(),
                                               tabObjectDisplayTaskHierarchyIndex[iTab][i],
                                               tabObjectDisplayObjectIndex[iTab][i][j]);
...
//            }
         ...
         buffer.AppendFormatted("            for (k = 0; k < TMath::Min(linearray->GetEntriesFast(),fNumberOfUserLines); k++) {\n");
         buffer.AppendFormatted("               line     = static_cast<TLine*>(linearray->At(k));\n");
         buffer.AppendFormatted("               userLine = static_cast<TLine*>(static_cast<TObjArray*>(static_cast<TObjArray*>(fUserLines->At(iDisplay))->At(chn))->At(k));\n");
...
Bool_t ROMEBuilder::WriteWindowCpp()
{
   ...
   for (i = 0; i < numOfTab; i++) {
      if (!tabUsed[i])
         continue;
      buffer.AppendFormatted("   fTabSwitches.%s%s = tabWindow;\n",tabName[i].Data(),tabSuffix[i].Data());
   ...
   for (i = 0; i < numOfTab; i++) {
      ...
      buffer.AppendFormatted("         if (fCurrentTabID == f%s%sTab->GetID()) {\n", tabName[i].Data(), tabSuffix[i].Data());
      if (tabHeredity[i].Length() > 0) {
         if (numOfMenu[tabHeredityIndex[i]] > 0) {
            buffer.AppendFormatted("            for (i = %d; i < %d + %d; i++) {\n", numOfMenu[i],numOfMenu[i],
                                   numOfMenu[tabHeredityIndex[i]]);
   ...
   buffer.AppendFormatted("            ROMEString newTitle = \"Argus - \";\n");
   for (i = 0; i < numOfTab; i++) {
      if (!tabUsed[i])
         continue;
      buffer.AppendFormatted("            if (fCurrentTabID == f%s%sTab->GetID()) {\n", tabName[i].Data(),
      ...
      buffer.AppendFormatted("               f%s%sTab->SetRegisteringActive(kFALSE);\n", tabName[i].Data(),
                             tabSuffix[i].Data());
      buffer.AppendFormatted("               newTitle += f%s%sTab->GetTitle();\n",
                             tabName[i].Data(), tabSuffix[i].Data());
      ...
      buffer.AppendFormatted("               newWindow->f%s%sTab->SetTabActive(kTRUE);\n",
                             tabName[i].Data(), tabSuffix[i].Data());
   ...
   for (i = 0; i < numOfTab; i++) {
      if (!tabUsed[i])
         continue;
      buffer.AppendFormatted("         // %s\n", tabName[i].Data());
      buffer.AppendFormatted("         if (fCurrentTabID == f%s%sTab->GetID() && param1 != f%s%sTab->GetID()) {\n", tabName[i].Data(),
   ...
   for (i = 0; i < numOfTab; i++) {
      ...
      buffer.AppendFormatted("         // %s\n", tabName[i].Data());
      buffer.AppendFormatted("         if (param1 == f%s%sTab->GetID()) {\n", tabName[i].Data(), tabSuffix[i].Data());
      if (tabHeredity[i].Length() > 0) {
         for (j = 0; j < numOfMenu[tabHeredityIndex[i]]; j++) {
      ...
      Int_t indx = i;
      do {
         buffer.AppendFormatted(" param1 != f%s%sTab->GetID() &&", tabName[indx].Data(), tabSuffix[i].Data());
   ...
   for (i = 0; i < numOfTab; i++) {
      if (!tabUsed[i])
         continue;
      buffer.AppendFormatted("   if (fTabSwitches.%s%s && fCurrentTabID == f%s%sTab->GetID())\n", tabName[i].Data(),
                             tabSuffix[i].Data(), tabName[i].Data(), tabSuffix[i].Data());
