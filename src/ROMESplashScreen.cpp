...
void CreateSplash(DWORD time,char* prog,char* vers,TString* author,int numAuthor)
{
    programName = prog;
    version = vers;
    authorNames = author;
...
LRESULT CALLBACK SplashWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    const char bmpDir[] = "\\icons\\rome.bmp";
    char FullBmpDir[128];
    char *RomeSysDir;
    int xScreen;
    int yScreen;
    ...
    switch (message)
    {
        case WM_CREATE:
            ...
            sprintf(FullBmpDir,"%s%s",RomeSysDir,bmpDir);
        ...
        case WM_PAINT:
            ...
            sprintf(buf,programName);
            SelectObject(hDC,fontProg);
            TextOut(hDC,bm.bmWidth/2,20,programName,strlen(buf));

            strcpy(buf,"generated by the");
            SelectObject(hDC,fontText);
            TextOut(hDC,bm.bmWidth/2,60,buf,strlen(buf));

            SetTextColor(hDC,0x00000b2);
            sprintf(buf,"ROME");
            SelectObject(hDC,fontROME);
            TextOut(hDC,bm.bmWidth/2,90,buf,strlen(buf));
            SetTextColor(hDC,0x0000000);

            sprintf(buf,"Environment");
            SelectObject(hDC,fontText);
            TextOut(hDC,bm.bmWidth/2,150,buf,strlen(buf));

            sprintf(buf,version);
            SelectObject(hDC,fontText);
            TextOut(hDC,bm.bmWidth/2,355,buf,strlen(buf));

            sprintf(buf,"Author of ROME : Matthias Schneebeli");
            SelectObject(hDC,fontAuthor);
            TextOut(hDC,bm.bmWidth/2,400,buf,strlen(buf));

            if (numberOfAuthors>0) {
               if (numberOfAuthors==1) sprintf(buf,"Author of the %s :",programName);
               else sprintf(buf,"Authors of the %s :",programName);
               SelectObject(hDC,fontAuthor);
               TextOut(hDC,bm.bmWidth/2,430,buf,strlen(buf));

               if (numberOfAuthors>5) {
                  for (i=0;i<numberOfAuthors;i++) {
                      sprintf(buf,authorNames[i]);
                      if (i+1<numberOfAuthors) sprintf(buf+strlen(buf),", %s",authorNames[(i+1)].Data());
                      SelectObject(hDC,fontAuthor);
                      TextOut(hDC,bm.bmWidth/2,450+20*(i/2),buf,strlen(buf));
                      i++;
                  }
               }
               else {
                  for (i=0;i<numberOfAuthors;i++) {
                      sprintf(buf,authorNames[i]);
                      SelectObject(hDC,fontAuthor);
                      TextOut(hDC,bm.bmWidth/2,450+20*i,buf,strlen(buf));
                  }
               }
            }
