...
Bool_t ROMEBuilder::WriteBaseTabH()
{
   ...
   for (Int_t iTab = 0; iTab < numOfTab; iTab++) {
      ...
      buffer.AppendFormatted("   Bool_t       IsActive() { return fActive; };\n");
   ...
   for (i=0;i<numOfFolder;i++) {
      ...
      if (folderDataBase[i] && !folderSupport[i]) {
         ...
         for (j=0;j<numOfValue[i];j++) {
            ...
            buffer.AppendFormatted(");\n");
            buffer.AppendFormatted("   if (!GetDataBase(gAnalyzer->Get%s_%sDBIndex())->Write(values,path,gAnalyzer->GetCurrentRunNumber())) {\n",folderName[i].Data(),valueName[i][j].Data());
...
Bool_t ROMEBuilder::WriteConfigToFormCpp() {
   ...
   buffer.AppendFormatted("   XMLToFormFrame *tempFrame;\n");
...
Bool_t ROMEBuilder::WriteConfigToFormTabs(ROMEString &buffer,ROMEConfigParameterGroup *parGroup,ROMEString pointer,ROMEString tabPointer,ROMEString configPointer)
{
   ...
   for (i=0;i<parGroup->GetNumberOfParameters();i++) {
      if (parGroup->GetParameterAt(i)->GetArraySize()=="1") {
         ...
         buffer.AppendFormatted("   tempFrame->AddElement(new XMLToFormElement(\"%s\",\"%s\",writeString.Data(),path.Data(),0,&entries));\n",parGroup->GetParameterAt(i)->GetWidgetType().Data(),parGroup->GetParameterAt(i)->GetName().Data());
      }
   }
   ...
   for (i=0;i<parGroup->GetNumberOfSubGroups();i++) {
      ...
      buffer.AppendFormatted("   tempFrame = %s;\n",pointer.Data());
      buffer.AppendFormatted("   tempFrame->AddSubFrame(new XMLToFormFrame(\"%s\",\"\",true,true,true,%d));\n",parGroup->GetSubGroupAt(i)->GetGroupName().Data(),i);
...
Bool_t ROMEBuilder::WriteMidasDAQCpp() {
   ...
   if (midas) {
      ...
      buffer.AppendFormatted("      ROMEPrint::Warning(\"Cannot write to task switches record.\\n\");\n");
      buffer.AppendFormatted("      return false;\n");
      buffer.AppendFormatted("   }\n");
      ...
      buffer.AppendFormatted("      ROMEPrint::Warning(\"Cannot open task switches record, probably other analyzer is using it\\n\");\n");
      buffer.AppendFormatted("      return false;\n");
      buffer.AppendFormatted("   }\n");
   }
   buffer.AppendFormatted("   return true;\n");
